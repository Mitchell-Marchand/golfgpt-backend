...
router.post("/create", authenticateUser, async (req, res) => {
    ...
    try {
        ...
        const prompt = `Based on the following match rules, generate a JSON object with:\n- \"displayName\": creative title\n- \"questions\": array of additional questions needed per hole, formatted as { \"question\": \"string\", \"options\": [\"array\", \"of\", \"choices\"] } (don't ever ask questions about player scores or strokes)\n\"strokes\": array with golferName and pops as an array of strokes the golfer gets each hole based on their handicap and/or other input from the user (if unknown at this point, put 0 for all) and the hole handicap/allocation, e.g. {"name": "Mitch", "pops": [{"allocation": 1, "strokes": 1}, {"alloaction": 2, "strokes": 1}, ..., {"allocation": 18, "strokes": 0}] \n\nRules:\n${rules}\n\nRespond ONLY with valid raw JSON.`;
...
router.post("/score/feedback", authenticateUser, async (req, res) => {
    const { matchId, feedback, holeNumber } = req.body;
