...
router.post("/create", authenticateUser, async (req, res) => {
    ...
    try {
        ...
        - "questions" (array): list of all additional questions to ask per hole (past tense) required to accurately 
        score the match based on the rules (don't do {question} on hole 1, {question} on hole 2, etc. - just the 
        question you'll repeat). Include proximity (closest to the hole not just on par 3s) if relevant for formats 
        like Scotch. Include 2x, 3x, 4x, 5x as options for increasing the bet if in a format that allows for it (like Scotch). Don't ask questions that you can get from the actual scores, as these will be provided. 
